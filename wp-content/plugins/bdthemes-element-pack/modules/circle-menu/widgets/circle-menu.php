...
class Circle_Menu extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Circle Menu', 'bdthemes-element-pack' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .bdt-circle-menu li.bdt-menu-icon' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			'circle_menu_icon_active_background',
			[
				'label'     => esc_html__( 'Background Color', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .bdt-circle-menu li.bdt-menu-icon:focus' => 'background-color: {{VALUE}};',
				],
			]
		);

		$this->add_control(
			'circle_menu_icon_active_border_color',
			[
				'label'     => esc_html__( 'Border Color', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::COLOR,
				'condition' => [
				...
				'selectors' => [
					'{{WRAPPER}} .bdt-circle-menu li.bdt-menu-icon:focus' => 'border-color: {{VALUE}};',
				],
			]
		);
	...
	public function render_loop_iconnav_list($list) {
	    ...
	    <li class="bdt-menu-icon">
			<a href="<?php echo esc_attr($list['iconnav_link']['url']); ?>" target="<?php echo esc_attr($target); ?>" <?php echo esc_attr($rel); ?> class="bdt-position-center" title="<?php echo esc_html($list['title']); ?>">
				<?php if ($list['icon']) : ?>
					<span>
	...
	protected function render_script($id) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {	  
                $('#<?php echo esc_attr($id); ?> .bdt-circle-menu').circleMenu({
					direction           : '<?php echo esc_attr($settings['direction']); ?>',
					item_diameter       : <?php echo esc_attr($settings['item_diameter']['size']); ?>,
					circle_radius       : <?php echo esc_attr($settings['circle_radius']['size']); ?>,
					angle               : {
					start               : 0,
					end                 : 90
					},
					speed               : <?php echo esc_attr($settings['speed']['size']); ?>,
					delay               : <?php echo esc_attr($settings['delay']['size']); ?>,
					step_out            : <?php echo esc_attr($settings['step_out']['size']); ?>,
					step_in             : <?php echo esc_attr($settings['step_in']['size']); ?>,
					trigger             : '<?php echo esc_attr($settings['trigger']); ?>',
					transition_function : '<?php echo esc_attr($settings['transition_function']); ?>',
                });
                $('#<?php echo esc_attr($id); ?> > div:first-child').css('display','block');
                $('#<?php echo esc_attr($id); ?> li:first-child').addClass('active');
	        });
        </script>
        <?php
	}
	...
	protected function render() {
		...
		$toggle_icon_position = ($settings['toggle_icon_position']) ? ' bdt-position-fixed bdt-position-' . $settings['toggle_icon_position'] : '';
		
		?>
		<div id="<?php echo esc_attr($id); ?>" class="bdt-circle-menu-container<?php echo esc_attr($toggle_icon_position); ?>">
            <ul class="bdt-circle-menu">
            	<li class="bdt-toggle-icon"><a href="javascript:void(0);" class="bdt-icon bdt-link-reset bdt-position-center" bdt-icon="icon: <?php echo esc_attr($toggle_icon); ?>; ratio: 1.1;" title="<?php esc_html_e('Click me to show menus.', 'bdthemes-element-pack'); ?>"></a></li>
				<?php
