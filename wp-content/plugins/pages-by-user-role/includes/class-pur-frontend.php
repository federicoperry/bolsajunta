	...
	*/
	...
	function template_redirect() {
		...
		if ( is_single() || is_page() ) {
			if ( ! $this->check_access( $object_id ) ) {
				$this->no_access_page( $object_id );
			}
		}
	}
	...
	function check_access( $object_id ) {
		if ( $this->is_blocked( $object_id ) ) {
			return false;
		} elseif ( $this->is_allowed( $object_id ) ) {
			return true;
		}
	...
	function is_blocked( $object_id ) {
		$blocked_roles = WP_PUR::blocked_roles( $object_id, 'get', 'post', '', true );
	...
	function is_allowed( $object_id ) {
		$allowed_roles = WP_PUR::available_roles( $object_id, 'get', 'post', '', true );
